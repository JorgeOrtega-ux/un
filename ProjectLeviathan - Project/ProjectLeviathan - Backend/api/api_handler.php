...
if ($action === 'get_chat_messages') {
    ...
    $limit = 100; // Lotes de 100 mensajes
    ...
    try {
        ...
        // 2. Obtener los mensajes del grupo, paginados
        $stmt_messages = $pdo->prepare(
            "SELECT
             ...
             WHERE m.group_uuid = :group_uuid
             ORDER BY m.sent_at DESC
             LIMIT :limit OFFSET :offset"
        );
        $stmt_messages->bindParam(':group_uuid', $groupUuid, PDO::PARAM_STR);
        $stmt_messages->bindParam(':limit', $limit, PDO::PARAM_INT);
        $stmt_messages->bindParam(':offset', $offset, PDO::PARAM_INT);
...
if ($action === 'send_message') {
    ...
    if (empty($groupUuid) || empty($messageText)) {
    ...
    try {
        ...
        $stmt_check->execute(['user_id' => $userId, 'group_uuid' => $groupUuid]);
        if ($stmt_check->rowCount() === 0) {
            echo json_encode(['success' => false, 'message' => 'No puedes enviar mensajes a un grupo al que no perteneces.']);
            exit;
        }

        // Insertar el mensaje en la base de datos
        $stmt_insert = $pdo->prepare(
            "INSERT INTO group_messages (uuid, group_uuid, user_id, message_text) 
             VALUES (:uuid, :group_uuid, :user_id, :message_text)"
        );
        $stmt_insert->execute([
            'uuid' => bin2hex(random_bytes(16)), // Generar un UUID para el mensaje
        ...
        error_log("API Error (send_message): " . $e->getMessage());
        echo json_encode(['success' => false, 'message' => 'Error del servidor al guardar el mensaje.']);
    }
    exit;
}
