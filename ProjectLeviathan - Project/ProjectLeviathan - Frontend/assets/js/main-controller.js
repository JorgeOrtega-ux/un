...
function initMainController() {
    ...
    // --- INICIO DE LA CORRECCIÓN: Usar los selectores genéricos ---
    ...
    // --- LÓGICA DE CHAT COMPLETA ---
    const loadChat = async (groupInfo) => {
        const chatTitle = document.getElementById('chat-group-title');
        ...
        try {
            const response = await fetch(`${window.PROJECT_CONFIG.apiUrl}?action=get_group_details&group_uuid=${groupInfo.uuid}`);
            const data = await response.json();

            if (data.success) {
                const realTitle = data.group.group_title;
                chatTitle.textContent = realTitle;
                ...
                currentChatGroupUuid = groupInfo.uuid;
                connectWebSocket(groupInfo.uuid);
                await loadMessageHistory(groupInfo.uuid);
            } else {
    ...
    const connectWebSocket = (groupUuid) => {
        const wsUrl = `ws://localhost:8765/chat/${groupUuid}`;
        websocket = new WebSocket(wsUrl);

        websocket.onopen = () => {
            console.log(`Conectado al WebSocket del grupo: ${groupUuid}`);
        };
        ...
        websocket.onmessage = (event) => {
            try {
                ...
                if (data.type === 'new_message') {
                    appendMessage(data.payload, true);
                }
            } catch (error) {
        ...
        websocket.onclose = () => {
            console.log(`Desconectado del WebSocket del grupo: ${groupUuid}`);
        };

        websocket.onerror = (error) => {
            console.error("Error de WebSocket:", error);
            alert('No se pudo conectar al servidor de chat. La mensajería en tiempo real no funcionará.');
        };
    };

    const loadMessageHistory = async (groupUuid, offset = 0) => {
        const messagesContainer = document.getElementById('chat-messages-container');
        if (offset === 0) {
            messagesContainer.innerHTML = '<div class="loader"></div>';
        }
        ...
        try {
            const response = await fetch(`${window.PROJECT_CONFIG.apiUrl}?action=get_chat_messages&group_uuid=${groupUuid}&offset=${offset}`);
            const data = await response.json();
            ...
            if (data.success) {
                ...
                messagesContainer.innerHTML = `<p>${data.message}</p>`;
            }
        } catch (error) {
    ...
    const appendMessage = (message, isNew) => {
        const messagesContainer = document.getElementById('chat-messages-container');
        const messageElement = document.createElement('div');
        const isSentByMe = message.user_id.toString() === window.PROJECT_CONFIG.userId.toString();

        messageElement.className = `message-bubble ${isSentByMe ? 'sent' : 'received'}`;
        messageElement.innerHTML = `
            <span class="message-info">${isSentByMe ? 'Tú' : message.username}</span>
    ...
    const handleSendMessage = async (messageText) => {
        ...
        try {
            ...
            const result = await response.json();
            if (!result.success) {
                console.error("Error al guardar el mensaje:", result.message);
                alert("No se pudo enviar tu mensaje. Inténtalo de nuevo.");
                return; // No enviar por WebSocket si no se pudo guardar
            }
        } catch (error) {
            console.error("Error de red al guardar el mensaje:", error);
            alert("Error de conexión. No se pudo enviar tu mensaje.");
            return;
        }

        // Enviar por WebSocket para retransmisión en tiempo real
        if (websocket && websocket.readyState === WebSocket.OPEN) {
            const messagePayload = {
                action: 'send_message',
                payload: {
                    user_id: window.PROJECT_CONFIG.userId,
                    username: window.PROJECT_CONFIG.username,
    ...
    function setupEventListeners() {
        ...
        // --- INICIO DE LA CORRECCIÓN: Escuchar clics en el contenedor genérico ---
